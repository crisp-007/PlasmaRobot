...
namespace {
std::mutex g_udp_joint_mutex;
std::array<float, 7> g_udp_joint_degrees{};
std::chrono::steady_clock::time_point g_udp_joint_updated_at{};
bool g_udp_joint_received = false;
}
...
void RmArm::Arm_MoveL_Offset_Callback(rm_ros_interfaces::msg::Moveloffset::SharedPtr msg)
{
    ...
    {
        res = Rm_Api.rm_movel_offset(robot_handle, pose, v, r,
                                     trajectory_connect, frame_type, block);
    }
    else
    {
        // rm_movel_offset is only available on fourth-generation controllers.
        // The generation-3 current-state query cannot parse some controller
        // replies. Build the target from the fresh UDP joints instead.
        std::array<float, 7> current_joints{};
        std::chrono::steady_clock::time_point joint_updated_at;
        bool joint_received = false;
        {
            std::lock_guard<std::mutex> lock(g_udp_joint_mutex);
            current_joints = g_udp_joint_degrees;
            joint_updated_at = g_udp_joint_updated_at;
            joint_received = g_udp_joint_received;
        }
        const auto joint_age = std::chrono::steady_clock::now() - joint_updated_at;
        if(joint_received && joint_age <= 500ms)
        {
            rm_pose_t target_pose{};
            const bool translation_only =
                std::abs(pose.euler.rx) < 1e-7 &&
                std::abs(pose.euler.ry) < 1e-7 &&
                std::abs(pose.euler.rz) < 1e-7;
            if(frame_type && translation_only)
            {
                target_pose = Rm_Api.rm_algo_cartesian_tool(
                    robot_handle, current_joints.data(),
                    pose.position.x, pose.position.y, pose.position.z);
            }
            else
            {
                const rm_pose_t current_pose = Rm_Api.rm_algo_forward_kinematics(
                    robot_handle, current_joints.data());
                float delta_pose[6] = {
                    pose.position.x,
                    pose.position.y,
                    pose.position.z,
                    pose.euler.rx * RAD_DEGREE,
                    pose.euler.ry * RAD_DEGREE,
                    pose.euler.rz * RAD_DEGREE
                };
                target_pose = Rm_Api.rm_algo_pose_move(
                    robot_handle, current_pose, delta_pose,
                    frame_type ? 1 : 0);
            }
            res = Rm_Api.rm_movel(robot_handle, target_pose, v, r,
        ...
        else
        {
            ...
            RCLCPP_ERROR(this->get_logger(),
                         "MoveL offset compatibility has no fresh UDP joint state");
        }
    }
    movel_data.data = res;
    if(movel_data.data == 0)
    ...
    else
    {
        ...
        this->MoveL_offset_Cmd_Result->publish(movel_result);
        RCLCPP_ERROR(this->get_logger(),
                     "MoveL offset error code is %d (controller generation %d)",
                     movel_data.data, controller_type);
    }
}
...
void RmArm::Set_Collision_Stage_Callback(const std_msgs::msg::UInt16::SharedPtr msg)
{
    std_msgs::msg::Bool result;
    std_msgs::msg::UInt16 stage;
    stage.data = std::numeric_limits<std::uint16_t>::max();
    if (msg->data > 8)
    {
        result.data = false;
        this->Set_Collision_Stage_Result->publish(result);
        this->Collision_Stage_Result->publish(stage);
        RCLCPP_ERROR(this->get_logger(), "Collision stage %u is outside [0, 8]", msg->data);
        return;
    }

    const int set_result = Rm_Api.rm_set_collision_state(robot_handle, static_cast<int>(msg->data));
    int actual_stage = -1;
    const int get_result = set_result == 0 ?
        Rm_Api.rm_get_collision_stage(robot_handle, &actual_stage) : set_result;
    result.data = set_result == 0 && get_result == 0 && actual_stage == static_cast<int>(msg->data);
    this->Set_Collision_Stage_Result->publish(result);
    if (!result.data)
    {
        this->Collision_Stage_Result->publish(stage);
        RCLCPP_ERROR(this->get_logger(),
            "Failed to set/verify collision stage %u (set=%d get=%d actual=%d)",
            msg->data, set_result, get_result, actual_stage);
        return;
    }

    stage.data = static_cast<std::uint16_t>(actual_stage);
    this->Collision_Stage_Result->publish(stage);
    RCLCPP_INFO(this->get_logger(), "Collision detection stage verified at %d", actual_stage);
}
...
void RmArm::Get_Arm_Version()
{
    ...
    if(res == 0)
    {
        ...
        RCLCPP_INFO(this->get_logger(), "controller generation = %d", controller_type);
...
void RmArm::Arm_Get_Current_Tool_Frame_Callback(const std_msgs::msg::Empty::SharedPtr msg)
{
    ...
    int32_t res;
    std_msgs::msg::String curr_frame;
    rm_ros_interfaces::msg::Toolframe frame_data;
    copy = msg;
    res = Rm_Api.rm_get_current_tool_frame(robot_handle, &tool_frame);
    frame_data.success = (res == 0);
    frame_data.error_code = res;
    if(res == 0)
    {
        curr_frame.data = tool_frame.frame_name;
        this->Get_Current_Tool_Frame_Result->publish(curr_frame);

        frame_data.frame_name = tool_frame.frame_name;
        frame_data.pose.position.x = tool_frame.pose.position.x;
        frame_data.pose.position.y = tool_frame.pose.position.y;
        frame_data.pose.position.z = tool_frame.pose.position.z;
        frame_data.euler_rad.x = tool_frame.pose.euler.rx;
        frame_data.euler_rad.y = tool_frame.pose.euler.ry;
        frame_data.euler_rad.z = tool_frame.pose.euler.rz;
        const rm_quat_t quaternion =
            Rm_Api.rm_algo_euler2quaternion(tool_frame.pose.euler);
        frame_data.pose.orientation.w = quaternion.w;
        frame_data.pose.orientation.x = quaternion.x;
        frame_data.pose.orientation.y = quaternion.y;
        frame_data.pose.orientation.z = quaternion.z;
        frame_data.payload_kg = tool_frame.payload;
        frame_data.center_of_mass_mm.x = tool_frame.x;
        frame_data.center_of_mass_mm.y = tool_frame.y;
        frame_data.center_of_mass_mm.z = tool_frame.z;
    }
    else
    {
        RCLCPP_INFO (this->get_logger(),
            "Arm_get_current_tool_frame_callback error code is %d\n", res);
    }
    this->Get_Current_Tool_Frame_Data_Result->publish(frame_data);
}
...
void Udp_Robot_Status_Callback(rm_realtime_arm_joint_state_t data)
{
    {
        std::lock_guard<std::mutex> lock(g_udp_joint_mutex);
        for(int i = 0; i < arm_dof_g; ++i)
            g_udp_joint_degrees[i] = data.joint_status.joint_position[i];
        g_udp_joint_updated_at = std::chrono::steady_clock::now();
        g_udp_joint_received = true;
    }
    for(int i = 0; i < 6; i++)
    {
...
/***********************************************************************运动配置**********************************************************************/
    ...
    Set_Collision_Stage_Result = this->create_publisher<std_msgs::msg::Bool>(
        "rm_driver/set_collision_stage_result", rclcpp::ParametersQoS());
    Collision_Stage_Result = this->create_publisher<std_msgs::msg::UInt16>(
        "rm_driver/collision_stage", rclcpp::ParametersQoS());
    Set_Collision_Stage_Cmd = this->create_subscription<std_msgs::msg::UInt16>(
        "rm_driver/set_collision_stage_cmd", rclcpp::ParametersQoS(),
        std::bind(&RmArm::Set_Collision_Stage_Callback,this,std::placeholders::_1),
        sub_opt2);
...
/************************************************************************坐标系指令*************************************************************/
    ...
    Get_Current_Tool_Frame_Data_Result = this->create_publisher<rm_ros_interfaces::msg::Toolframe>("rm_driver/get_current_tool_frame_data_result", rclcpp::ParametersQoS());
