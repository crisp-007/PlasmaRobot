...
void CropTask::run()
{
    try {
        ...
        if (m_params.mode == 1) { // Crop_Rect = 1
        ...
        else if (m_params.mode == 2) { // Crop_Poly = 2
...
void RosToVtkTask::run()
{
    try {
        ...
        for (size_t i = 0; i < m_msg->fields.size(); ++i) {
            if (m_msg->fields[i].name == "x") x_idx = m_msg->fields[i].offset;
            else if (m_msg->fields[i].name == "y") y_idx = m_msg->fields[i].offset;
            else if (m_msg->fields[i].name == "z") z_idx = m_msg->fields[i].offset;
            else if (m_msg->fields[i].name == "rgb" || m_msg->fields[i].name == "rgba") rgb_idx = m_msg->fields[i].offset;
        }

        if (x_idx == -1 || y_idx == -1 || z_idx == -1) {
            QMetaObject::invokeMethod(m_receiver, "onTaskError", Q_ARG(QString, "Missing XYZ fields in ROS pointcloud"));
            return;
        }
        ...
        // if (!logged) {
        //     qDebug() << "PointCloud Fields Found - X:" << x_idx << " Y:" << y_idx << " Z:" << z_idx << " RGB:" << rgb_idx;
        //     logged = true;
        // }

        // 3. 遍历二进制数据
        const uint8_t* ptr = m_msg->data.data();
        uint32_t point_step = m_msg->point_step;

        for (size_t i = 0; i < numPoints; ++i) {
            const uint8_t* point_ptr = ptr + i * point_step;
            
            float x = *reinterpret_cast<const float*>(point_ptr + x_idx);
            float y = *reinterpret_cast<const float*>(point_ptr + y_idx);
            float z = *reinterpret_cast<const float*>(point_ptr + z_idx);

            // 检查有效性 (非 NaN/Inf)
            if (std::isfinite(x) && std::isfinite(y) && std::isfinite(z)) {
                // 坐标变换：反转Y和Z轴，修正倒置问题 (Camera Frame -> VTK World Frame)
                points->InsertNextPoint(x, -y, -z);

                // 处理颜色
                if (rgb_idx != -1) {
                    const uint8_t* rgb_ptr = point_ptr + rgb_idx;
                    uint8_t r = rgb_ptr[2];
                    uint8_t g = rgb_ptr[1];
                    uint8_t b = rgb_ptr[0];
                    colors->InsertNextTuple3(r, g, b);
                } else {
                    colors->InsertNextTuple3(255, 255, 255); // 默认白色
                }
            }
        }
