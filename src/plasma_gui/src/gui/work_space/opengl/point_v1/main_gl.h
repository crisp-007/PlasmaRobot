...
public:
    ...
    // --- 可视化设置 ---
    // 设置网格属性：gridSize为网格总边长，step为单个格子边长，color为颜色
    void SetGridProperties(double gridSize, double step, const QColor &color);
    // 设置背景颜色（渐变）
...
private:
    ...
    vtkRecti m_selectedRect;
    bool m_hasRectSelection = false;

    // --- 成员变量: 多边形裁剪可视化 ---
    vtkSmartPointer<vtkActor2D> m_polyActor;
    ...
    bool m_isFirstCloudReceived = true; // 标志位：是否是第一次接收到点云
};
...
// 1. 点云平移样式 (右键平移)
class PointcloudMoveStyle : public vtkInteractorStyleTrackballCamera {
public:
    static PointcloudMoveStyle* New();
    vtkTypeMacro(PointcloudMoveStyle, vtkInteractorStyleTrackballCamera);
