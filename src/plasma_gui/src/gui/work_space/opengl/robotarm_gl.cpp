...
RobotArmgl::RobotArmgl(QWidget *parent)
    : QOpenGLWidget(parent), show_no_signal(true)
{
    ...
    qDebug() << "Robot arm - App directory:" << appDir;
    
    // 尝试多个可能的路径
    QStringList possiblePaths = {
        QDir(appDir).absoluteFilePath("image/robot_logo.png"),                    // 同级image文件夹
        QDir(appDir).absoluteFilePath("../../image/robot_logo.png"),             // 项目根目录的image文件夹
        QDir(appDir).absoluteFilePath("../../../image/robot_logo.png"),          // 上级目录的image文件夹
        "d:/WorkSpace/CodeSpace/Qt/test1/image/robot_logo.png"                   // 绝对路径
    };
    
    bool loaded = false;
    for (const QString& path : possiblePaths) {
        QString cleanPath = QDir::cleanPath(path);
        qDebug() << "Trying to load robot arm image from:" << cleanPath;
        loaded = no_signal_image.load(cleanPath);
        if (loaded) {
            qDebug() << "Successfully loaded robot arm image from:" << cleanPath;
            qDebug() << "Robot arm image size:" << no_signal_image.size();
            break;
        }
    }
    
    if (!loaded) {
        qDebug() << "Failed to load robot arm image from all attempted paths";
    }
}

RobotArmgl::~RobotArmgl()
{
}

void RobotArmgl::SetupFont()
{
    // 字体将在paintGL中根据窗口尺寸动态调整
    text_font.setPointSize(OpenGLDisplay::NO_SIGNAL_BASE_FONT_SIZE);
}

void RobotArmgl::SetShowNoSignal(bool show)
{
    show_no_signal = show;
    update();
}

void RobotArmgl::SetCustomText(const QString &text)
{
    custom_text = text;
    update();
}

void RobotArmgl::initializeGL()
{
...
void RobotArmgl::paintGL()
{
    ...
    if (show_no_signal) {
        ...
        RenderRobotArmContent();
    }
}

void RobotArmgl::resizeGL(int w, int h)
{
    glViewport(0, 0, w, h);
}

void RobotArmgl::RenderNoSignalText(QPainter& painter)
{
    // 根据窗口尺寸动态调整字体
...
void RobotArmgl::RenderRobotArmContent()
{
    // 这里将来会添加机械臂的OpenGL渲染内容
    // 例如：3D机械臂模型、运动轨迹等
}

void RobotArmgl::DrawRoundedBorder(QPainter& painter)
{
    // 设置边框画笔
